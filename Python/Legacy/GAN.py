...
class CGAN():
    ...
    # Returns
      # Tuple of Numpy arrays: `(x_train, y_train), (x_test, y_test)`.
          ...
          with np.load(TRAIN_DATA_PATH, allow_pickle=True) as f:
    ...
    def create_bin(self):
        ...
        if not os.path.isfile(TRAIN_DATA_PATH):
            ...
            for filename in tqdm(os.listdir(IMAGE_PATH)):
                ...
                image = Image.open(PATH).resize((28, 28),Image.ANTIALIAS) #rows/cols as per gan 
                training_data.append(np.asarray(image))
            training_data = np.reshape(training_data, (-1, 28, 28, 1)) #rows cols and channels
